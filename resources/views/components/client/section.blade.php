...
@if ($data->module->clients ?? false)
    ...
    <x-core.layout :$button_header :$button_url :$is_filled :$is_section_filled_inverted :$with_padding
        ...
        @if ($is_heading_visible ?? false)
            @if ($data->layout->customer['section_title'] ?? false)
                <x-slot name="module_title">
                    {!! $data->layout->customer['section_title'] ?? null !!}
                </x-slot>
            @endif
            @if ($data->layout->customer['section_subtitle'] ?? false)
                <x-slot name="module_subtitle">
                    {!! $data->layout->customer['section_subtitle'] ?? null !!}
                </x-slot>
            @endif
        @endif
