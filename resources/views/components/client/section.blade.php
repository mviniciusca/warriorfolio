...
@if ($module->clients)
    <x-core.layout :$with_padding :$is_filled :$button_header :$button_url :module_name="'customers'">
        ...
        <section id="clients" class="flex flex-wrap content-center items-center justify-center gap-8 pb-12">
            @foreach ($customers as $client)
                <div
                    class="flex min-h-24 w-1/4 items-center rounded-md border bg-white opacity-50 grayscale transition-all duration-100 hover:opacity-100 hover:grayscale-0 dark:border-secondary-700 dark:bg-secondary-900 dark:opacity-60 dark:hover:opacity-100 md:w-1/4 lg:w-1/6">
                    <x-curator-glider :media="$client->logo"
                        class="mx-auto max-w-24 object-contain object-center p-1 invert-0 dark:invert" />
                </div>
            @endforeach
        </section>
