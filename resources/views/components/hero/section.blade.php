        ...
        <section>
            <div class="mx-auto flex flex-col items-center justify-center">
                ...
                @if (data_get($hero, 'hero.slider_is_active'))
                    <div class="flex max-w-7xl mb-16 mt-4 mx-auto flex-wrap justify-around gap-4">
                        @foreach (collect($hero->hero['slider_content'])->flatten(1) as $item)
                            <img class="{{ $hero->hero['is_invert'] ? 'dark:invert' : null }} opacity-50 hover:opacity-100 transition-all px-4 duration-100"
