...
@if ($hero->hero['is_active'])
    @if ($hero->hero['is_pattern_bg'])
        <div id="hero-background-image"
            class="{{ $hero->hero['bg_size'] ?? 'bg-cover' }} {{ $hero->hero['bg_position'] ?? 'bg-center' }} {{ $hero->hero['bg_repeat'] ?? 'bg-no-repeat' }} bg-hero absolute -z-50 m-auto h-[600px] w-full bg-fixed opacity-5 filter dark:opacity-10 dark:invert"
