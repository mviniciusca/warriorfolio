...
@if ($style === 'filled' && !$is_section_filled_inverted && !$color)
    <button
        {{ $attributes->class([
            'bg-black hover:bg-secondary-700 text-xs md:text-sm dark:hover:bg-secondary-200 transition-all duration-300 text-white dark:bg-white dark:text-black flex flex-wrap gap-2 border border-transparent py-2 px-4 rounded-md items-center active:opacity-30 justify-center',
        ]) }}
        type="{{ $type }}">
        <span>
            {{ $slot }}
