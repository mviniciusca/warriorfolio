...
'is_animated' => null,
'is_border' => null,
'is_center' => null, // deprecated v2.1.5
'is_color_icon' => null,
...
<x-core.layout :$is_filled :$is_section_filled_inverted :$button_header :$button_url :$is_center>
    ...
    <div class="mx-auto{{ $with_padding ? 'py-8' : 'py-12'}}">
        <div
            ...
            @if ($item['link'] ?? false)
            <a class="block w-full h-full cursor-pointer transition-all duration-300 hover:opacity-50 active:opacity-20"
                target="{{ $item['is_new_window'] ? '_blank' : '_self' }}" href="{{ $item['link'] ?? '#' }}">
                @endif
                <div
                    class="flex flex-col justify-between w-full h-full {{ $columns != 1 ? 'min-h-24' : 'min-h-10' }} {{ ($is_card_filled && (!$is_section_filled_inverted ?? false)) ? 'dark:bg-secondary-900/70 bg-secondary-50/70' : '' }} {{ ($is_card_filled && ($is_section_filled_inverted ?? false)) ? 'bg-secondary-900/70 dark:bg-secondary-50/70' : '' }} {{ ($is_border && ($is_section_filled_inverted ?? false)) ? 'border dark:border-secondary-800/20 border-secondary-300/10' : '' }} {{ ($is_border && (!$is_section_filled_inverted ?? false)) ? 'border dark:border-secondary-800/70 border-secondary-300' : '' }} icon-card duration-300 overflow-hidden rounded-lg bg-contain bg-top bg-no-repeat p-4 transition-all">
                    @if ($is_light_fx)
                    ...
                    @endif
                    <div class="flex {{ $is_content_center ? 'justify-center' : 'justify-start' }} -mt-0 mb-2">
                        <ion-icon
                    ...
                    <div class="flex flex-col flex-grow">
                        ...
                        <p class="{{ $columns != 1 ? 'py-2' : 'pb-2' }} text-sm leading-snug opacity-80"> {!!
                            $item['description'] !!}</p>
                    </div>
                </div>
                @if ($item['link'] ?? false)
