...
@if ($is_active)
    <x-core.layout :$button_header :$button_url :$is_center :$is_filled :$is_section_filled_inverted>
        @if (($module_title || $module_subtitle) && $is_heading_active)
            <div>
                <x-slot class="py-8 text-center" name="module_title">
                    {!! $module_title !!}
                </x-slot>
            ...
            <div
                ...
                @foreach ($features as $key => $item)
                    @if (!isset($item['is_card_hidden']) || empty($item['is_card_hidden']))
                        @if ($item['link'] ?? false)
                            <a class="h-full cursor-pointer transition-all duration-300 hover:opacity-50 active:opacity-20"
                                href="{{ $item['link'] ?? '#' }}"
                                target="{{ $item['is_new_window'] ? '_blank' : '_self' }}">
                        @endif
                        <div
                            class="{{ $columns != 1 ? 'min-h-24' : 'min-h-10' }} {{ $is_card_filled && (!$is_section_filled_inverted ?? false) ? 'dark:bg-secondary-900/70 bg-secondary-50/70' : '' }} {{ $is_card_filled && ($is_section_filled_inverted ?? false) ? 'bg-secondary-900/70 dark:bg-secondary-50/70' : '' }} {{ $is_border && ($is_section_filled_inverted ?? false) ? 'border dark:border-secondary-800/20 border-secondary-300/10' : '' }} {{ $is_border && (!$is_section_filled_inverted ?? false) ? 'border dark:border-secondary-800/70 border-secondary-300' : '' }} icon-card h-full overflow-hidden rounded-lg bg-contain bg-top bg-no-repeat p-4 transition-all duration-300">
                            @if ($is_light_fx)
