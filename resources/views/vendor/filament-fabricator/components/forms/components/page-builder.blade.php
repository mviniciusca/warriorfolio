...
// Tenta obter showSidebar ou usa true como padrão
...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div x-data="{}" {{ $attributes ->merge($getExtraAttributes(), escape: false)
        ->class(['fi-fo-builder'])
        }}
        >
                ...
                <x-filament::section heading="Modules" description="Click to add a new module" icon="heroicon-o-cube"
                    ...
                    <div>
                        <div class="space-y-2">
                            ...
                            $wireClickAction = "mountFormComponentAction('{$statePath}', '{$addAction->getName()}',
                            {$wireClickActionArguments})";
                            @endphp
                            <button type="button"
                                class="flex items-center gap-3 w-full p-2 text-left text-sm rounded-md border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                                wire:click="{{ $wireClickAction }}">
                                @if ($icon = $block->getIcon())
                                <x-filament::icon :icon="$icon"
                                    class="h-4 w-4 text-gray-400 dark:text-gray-500 flex-shrink-0" />
                                @endif
                                <span class="truncate">{{ $block->getLabel() }}</span>
                            </button>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <ul x-sortable data-sortable-animation-duration="{{ $getReorderAnimationDuration() }}"
                        ...
                        <li wire:key="{{ $this->getId() }}.{{ $item->getStatePath() }}.{{ $field::class }}.item" x-data="{
                            ...
                            x-on:expand="isCollapsed = false" x-sortable-item="{{ $uuid }}"
                            ...
                            @if ($isReorderableWithDragAndDrop || $isReorderableWithButtons || $hasBlockLabels ||
                            $isCloneable || $isDeletable || $isCollapsible || count($visibleExtraItemActions))
                            <div class="fi-fo-builder-item-header flex items-center gap-x-3 overflow-hidden px-4 py-3">
                            ...
                            <div class="flex w-full justify-center opacity-0 transition duration-75 hover:opacity-100">
                                <div class="fi-fo-builder-block-picker-ctn rounded-lg bg-white dark:bg-gray-900">
                                    ...
                                    <x-filament-fabricator::forms.components.page-builder.dropdown-block-picker
                                        :action="$addBetweenAction" :after-item="$uuid" :columns="$blockPickerColumns"
                                        :blocks="$blockPickerBlocks" :state-path="$statePath"
                    ...
                    @if ($isAddable)
                    @if ($blockPickerStyle === BlockPickerStyle::Dropdown)
                    <x-filament-fabricator::forms.components.page-builder.dropdown-block-picker :action="$addAction"
                        :blocks="$blockPickerBlocks" :columns="$blockPickerColumns" :state-path="$statePath"
                        :width="$blockPickerWidth" class="flex justify-center">
                        <x-slot name="trigger">
                            {{ $addAction }}
                        </x-slot>
                    </x-filament-fabricator::forms.components.page-builder.dropdown-block-picker>
                    @elseif ($blockPickerStyle === BlockPickerStyle::Modal)
                    <x-filament-fabricator::forms.components.page-builder.modal-block-picker :action="$addAction"
                        :blocks="$blockPickerBlocks" :columns="$blockPickerColumns" :state-path="$statePath"
                        :width="$blockPickerWidth" class="flex justify-center">
                        <x-slot name="trigger">
                            {{ $addAction }}
