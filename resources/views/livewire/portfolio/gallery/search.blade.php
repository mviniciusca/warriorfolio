<div class="relative">
    <form wire:submit.prevent="submit" class="relative">
        ...
        <input type="text" wire:model.live.debounce.500ms="search" placeholder="{{ __('Search projects...') }}"
            ...
            :class="{'border-primary-500 dark:border-primary-400': $wire.searchActive}">
    ...
    @if(strlen($search) > 0 && strlen($search) < 5) <div class="mt-1 text-xs text-amber-600 dark:text-amber-400">
        {{ __('Type at least 5 characters to search') }}
</div>
@endif

@if($searchActive)
<div class="mt-1 text-xs text-primary-600 dark:text-primary-400 flex items-center">
